...
files <- fs::dir_ls("./analysis_0.1_distance/DEG_lists/", regexp = "DEG_summary.txt",
                    recurse = T)
...
###################################################
# Plot
pctdist10 <- ggplot(degs, aes(x = Condition, y = DEGs, fill = Timepoint)) +
...
files <- fs::dir_ls("./analysis_0.15_distance/DEG_lists/", regexp = "DEG_summary.txt",
                    recurse = T)
files <- as.vector(files)
...
###################################################
# Plot
pctdist15 <- ggplot(degs, aes(x = Condition, y = DEGs, fill = Timepoint)) +
  geom_bar(stat="identity", position="dodge") +
  facet_wrap(~Chemical, scales = "free_x") +
  theme(axis.text.x = element_blank()) # element_text(angle = 90))
