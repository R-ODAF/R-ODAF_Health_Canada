...
This repository can be used to analyze transcriptomic datasets generated by [TempO-seq](https://www.biospyder.com/technology/), [DRUG-Seq](https://www.nature.com/articles/s41467-018-06500-x)] or RNA-seq. It generates:
...
_Note on Snakemake versions : Snakemake v8.0 and above introduced [changes to the snakemake command used to run the workflow](https://snakemake.readthedocs.io/en/stable/project_info/history.html#detailed-breaking-changes). The commands given in this README apply to v8.0 and above. If you are using an earlier version of Snakemake, replace 
`--software-deployment-method conda` with `--use-conda`_
...
Note that this workflow requires the expected input data to exist before snakemake is run. If you want to install conda environments before preparing your input files, download the test dataset as explained in the [Running Tests](#running-tests) section below.
...
Download the test dataset from our [test data repository](https://github.com/EHSRB-BSRSE-Bioinformatics/test-data). This will create a new inputs directory already populated with the temposeq test data.
...
- Raw fastq files (inputs/raw/) - file names must match the sample_ID column in the metadata file.
...
We strongly recommend using snakemake's [dry run functionality](https://snakemake.readthedocs.io/en/v7.32.2/executing/cli.html) to check for errors before beginning an actual run.
...
`rm output/.rodaf_internal/reports_complete`

# Output files

## Preprocessing
- count_table.tsv (under output/processed). A matrix of gene (RNA-seq, DRUG-seq) or probe (TempO-seq) counts per sample, in csv format.

## Quality control

- A [MultiQC](https://multiqc.info/) report
- An HMTL study-wide QC report showing pass/fail for samples according to filtering criteria defined by the [R-ODAF](https://www.sciencedirect.com/science/article/pii/S0273230022000307) and [Harrill et al. (2021)](https://pubmed.ncbi.nlm.nih.gov/33538836/) . The thresholds for these filters are set in the config file.
- An update metadata file containing only samples that passed QC

## Differentially expressed genes

An subdirectory is created within output/analysis each time a new differential analysis is run.

Each analysis folder contains the following subdirectories: summary, reports, DEG_lists, RData, Pipeline_record, (optional) BMD_and_biomarker_files, (optional) pathway_analysis. 

__summary__ contains:
- an html report showing a basic summary of QC parameters, samples passing/failing QC, number of DEGs per contrast, and (optional) biomarker calls
- *DEG_counts.txt - a table with number of upregulated, downregulated, and total DEGs per contrast
...
__pathway_analysis__ contains txt files output during pathway analysis
...
This workflow is a work in progress, and problems are often encountered by even highly experienced users. Please reach out if you have questions or issues, and feel free to submit an [issue](https://github.com/R-ODAF/R-ODAF_Health_Canada/issues) or PR with any suggestions you might have.
