...
############################################################################
paths <- R.ODAF.utils::set_up_project_paths(params)
...
tech_ctrl_names <- exp_metadata %>% 
  dplyr::filter(!!ensym(params[["technical_control"]]) == T) %>%
  dplyr::pull(original_names)

sample_names <- exp_metadata %>% 
  dplyr::filter(!!ensym(params[["technical_control"]]) == F) %>%
  dplyr::pull(original_names)

ref_samples <- exp_metadata %>% 
  dplyr::filter(!!ensym(params[["reference_rna"]]) == T) %>%
  dplyr::pull(original_names)
...
cor_distribution_all <- left_join(cor_df1, cor_df2) %>%
    mutate(mean = rowMeans(across(where(is.numeric)))) %>%
    dplyr::select(mean, original_names, params[["treatment_var"]])
cor_plot <- ggplot(cor_distribution_all, aes(!!ensym(params[["treatment_var"]]), mean)) +
...
metadata_outliers_removed <- exp_metadata[!exp_metadata$original_names %in% outliers,] %>%
  filter(!!ensym(params[["technical_control"]]) == F)
...
QAQC_metadata_subset <- QAQC_annotated %>%
  dplyr::select(unlist(exp_groups), !!ensym(params[["batch_var"]]),
  ...
  for (i in seq_along(exp_groups)[1:length(exp_groups)]) {
    ...
    p <- ggpairs(QAQC_metadata_subset,
                 ...
                 columns = c(covariate_cols,
                           "NMR_data", "FMR_data", "Ncov5_data", "Nsig80_data", "Gini_data"),
                 ggplot2::aes(colour = !!ensym(params[["batch_var"]])))
    print(p)
  }
} else {
  if (length(exp_groups) <= 1 & !is.na(params[["batch_var"]])) {
...
plate_plot <- ggplot(QAQC_annotated, aes(x = factor(!!ensym(colname)),
                                         ...
                                         name = original_names,
                                         treat = !!ensym(params[["treatment_var"]]),
                                         dose = !!ensym(params[["dose"]]))) +
  ...
  {if (!is.na(params[["batch_var"]]))  facet_wrap(paste0("~",params[["batch_var"]]), ncol = 2)}
#  {if (!is.na(params[["batch_var"]]))  facet_rep_wrap(paste0("~",params[["batch_var"]]), repeat.tick.labels = T)}
...
```{r fig.height = as.numeric(custom_fig_height), eval = are_there_plates}
(plate_plot +
  geom_tile(mapping = aes(fill = log(!!ensym(params[["dose"]]) + 1)),
            data = QAQC_annotated) +   
...
QAQC_failed_kable <- outlier_metadata %>%
  dplyr::select(original_names, !!ensym(params[["treatment_var"]]), !!ensym(params[["dendro_color_by"]])) %>%
...
knitr::kable(QAQC_annotated %>%
               dplyr::group_by(!!ensym(params[["treatment_var"]]), !!ensym(params[["dose"]]), Any) %>% # Could be better to somehow use exp_groups[[1]], but not sure how to get it working as expected
...
QAQC_annotated %>%
  dplyr::group_by(dendrogram, NMR, FMR, Ncov5, Nsig80, Gini, Any, !!ensym(params[["treatment_var"]])) %>%
  tally() %>%
...
plot_batches <- ggplot(QAQC_annotated, aes(x = !!ensym(params[["treatment_var"]]), color = !!ensym(params[["treatment_var"]]))) +
...
plot_dose_effect <- ggplot(QAQC_annotated, aes(x = factor(!!ensym(params[["dose"]])), color = factor(!!ensym(params[["dose"]])))) +
