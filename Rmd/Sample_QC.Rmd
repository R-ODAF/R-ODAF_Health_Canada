...
If you included ERCC spike-in RNA [NOTE add link/citation], reads matching the ERCC sequences should represent 3-5% of the obtained mapped reads.
...
Note that these plots are made using the data from count_table.tsv, and reflect whatever UMI deduplication method was used to produce that count table.
(As of June 12, 2026, the authoritative UMI dedup method is 1MM_Directional.)
...
# To plot just one UMI method for simplicity
# dedup_df <- dedup_df %>% dplyr::filter(umiMethod == "Directional")

# NOTE this probably should be box plots, not individual samples
# But I'm not sure how to do that nicely with the different UMI methods

# Plot
ggplot(dedup_df, aes(x = sample, y = ratio)) +
  geom_point(aes(color = umiMethod)) +
  facet_wrap(~Library, scales = "free_x") +
  ylim(0,1) +
  labs(y = "Percent unique reads (dedup ratio)",) +
  theme(axis.text.x = element_blank())

# ggplot(dedup_df, aes(x = sample, y = sumreads_dedup)) +
#   geom_point(aes(color = umiMethod)) +
#   facet_wrap(~Library, scales = "free_x") +
#   labs(y = "Number of mapped deduplicated reads",) +
#   theme(axis.text.x = element_blank(),
#   (text=element_text(size=30))) +
#   scale_y_continuous(labels = function(x) format(x, scientific = TRUE))

# ggplot(dedup_df, aes(x = sample, y = sumreads_nodedup)) +
#   geom_point() +
#   facet_wrap(~Library, scales = "free_x") +
#   labs(y = "Number of mapped reads (no deduplication)",) +
#   theme(axis.text.x = element_blank())
