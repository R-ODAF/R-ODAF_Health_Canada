...
collapse_reps <- function(filepath) {
  ...
  df <- df %>% tidyr::pivot_longer(cols = -c("X"),
...
sort_hclust <- function(dend, method = "average") {
  # Iterative dendrogram sorting to avoid stack overflow
  # This is a simplified version - may need refinement for your use case
  
  # If dendsort fails, just return the dendrogram as-is
  tryCatch({
    as.hclust(dendsort::dendsort(as.dendrogram(dend)), method = method)
  }, error = function(e) {
    warning("dendsort iterative fallback: returning unsorted dendrogram")
    dend
  })
}
...
if (params$collapse_replicates == T) {
  ...
  if (params$platform == "DRUG-Seq") {
    # MultiQC file is quite  different for DrugSeq vs. TempO-Seq and RNA-Seq
    multiqc_general_stats <- read.table(unz(multiQCFile, "multiqc_samtools_stats.txt"), 
                                      header = TRUE, sep = '\t')
    mu <- data.frame(Sample = multiqc_general_stats$Sample, 
                   FMR = multiqc_general_stats$reads_mapped_percent/100)
  } else {
...
Details on each sample can be seen in the MultiQC report.
...
# Stat column names are different for DRUG-Seq vs. TempO-Seq or RNA-Seq
if (params$platform == "DRUG-Seq") {
  ggplot(summary_stats %>%
         dplyr::filter(name %in% c("raw_total_sequences", "reads_mapped",
                       "reads_mapped_percent", "mismatches",
...
The plot is interactive. Click on a point to highlight it. Double-click to reset the plot.

```{r pca_plot, fig.width = 9, fig.height = 8}
...
# Extract column names from config parameters
expgroup1 <- params$exp_groups[[1]][1]
exp_group2 <- if (length(params$exp_groups[[1]]) > 1) params$exp_groups[[1]][2] else NULL

# Generalize the PCA code
if (!is.null(exp_group2)) {
  pca_data <- pca_data %>%
    group_by(across(all_of(expgroup1))) %>%
    mutate(level = dense_rank(across(all_of(exp_group2)))) %>%
    ungroup()
  
  # Convert level to a factor
  pca_data$level <- as.factor(pca_data$level)
  
  # Create a combined label for the legend
  pca_data$combined_label <- paste(pca_data[[expgroup1]], pca_data$level, sep = "_")
} else {
  pca_data$level <- as.factor(1)  # Use a single level if exp_group2 is not provided
  pca_data$combined_label <- pca_data[[expgroup1]]
}

# Plot PCA with ggplot2
p_pca <- ggplot(pca_data, aes(x = PC1, y = PC2, color = !!sym(expgroup1), shape = level)) +
  geom_point(size = 3) +
...
p_pca_plotly <- plotly::ggplotly(p_pca, tooltip = "text")
...
dendro_before_all_samples <- sort_hclust(stats::hclust(as.dist(dendro_before_all_samples), method = "average"))
...
A `r params$clust_method` distance of `r params$group_tree_height_cutoff` is used to cut the tree and separate outliers.
```{r group-dendrograms}
...
## Metadata of samples removed by filters
...
## Number of filtered samples in each group
...
knitr::kable(QAQC_annotated %>%
               ...
               tidyr::pivot_wider(names_from = Any, values_from = n, values_fill = 0)) %>% # wide table instead of long
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  kableExtra::scroll_box(width = "100%", height = "480px")
```


## Pass/fail table for samples removed by filters
...
# Set custom figure dimensions based on the number of plates
is_even <- function(number) {
  return(number %% 2 == 0)
}

if (batch_length == 1) {
  custom_fig_height <- 2.75
  custom_fig_width <- 4
} else if (batch_length > 1) {
  custom_fig_height <- 2.75 * ceiling(batch_length / 2)
  custom_fig_width <- 8
}
```
...
# Helper function: Scale plate plot colours around the threshold values
# `trans = "log10"` requires the midpoint in log space; symmetric = TRUE gives
# equal colour range on both sides so hue intensity is comparable above/below.
scale_threshold <- function(x, threshold, trans = "identity",
                            low = "#B2182B", mid = "#FFFFBF", high = "#2166AC",
                            symmetric = FALSE, aesthetics = "colour", ...) {
  f <- if (trans == "log10") function(v) log10(pmax(v, 1)) else identity
  m <- f(threshold)
  rng <- range(f(x), na.rm = TRUE)
  lims <- if (symmetric) {
    half <- max(abs(rng - m))
    c(m - half, m + half)
  } else {
    c(min(rng[1], m), max(rng[2], m))
  }
  lims <- lims + c(-1, 1) * diff(lims) * 0.001  # keep extreme breaks inside the limits
  ggplot2::scale_colour_gradient2(
    low = low, mid = mid, high = high,
    midpoint = m,
    limits = if (trans == "log10") 10^lims else lims,
    trans = trans,
...
(plate_plot +
  geom_point(mapping = aes(fill = NMR_data, val = NMR_data), data = QAQC_annotated,
             shape = 21, size = 3, colour = "grey20", stroke = 0.5) +
  # Colour scale around the NMR threshold
  scale_threshold(QAQC_annotated$NMR_data,
                  threshold = params$nmr_threshold,
                  trans = "log10",
                  aesthetics = "fill",
                  name = "Mapped\nreads",
                  breaks = c(min(QAQC_annotated$NMR_data), params$nmr_threshold,
                             max(QAQC_annotated$NMR_data)),
                  labels = scales::comma) +
...
```{r fig.height = as.numeric(custom_fig_height), fig.width = custom_fig_width, eval = are_there_plates}
(plate_plot +
  geom_point(mapping = aes(fill = FMR_data, val = FMR_data), data = QAQC_annotated,
           shape = 21, size = 3, colour = "grey20", stroke = 0.5) +
  # Colour scale around the FMR threshold
  scale_threshold(QAQC_annotated$FMR_data, 
                  threshold = params$align_threshold, 
                  aesthetics = "fill",
                  name = "FMR", 
                  breaks = c(0, params$align_threshold, 1), labels = scales::percent) +
...
```{r fig.height = as.numeric(custom_fig_height), fig.width = custom_fig_width, eval = are_there_plates}
(plate_plot +
  geom_point(mapping = aes(fill = Nsig80_data, val = Nsig80_data), data = QAQC_annotated, size = 3) +
  ggtitle("Nsig80")) %>%
  ggplotly(tooltip = c("name", "treat", "dose", "val"))
```
...
```{r fig.height = as.numeric(custom_fig_height), fig.width = custom_fig_width, eval = are_there_plates}
(plate_plot +
  geom_point(mapping = aes(fill = Ncov5_data, val = Ncov5_data), data = QAQC_annotated, size = 3) +
  ggtitle("Ncov5")) %>%
  ggplotly(tooltip = c("name", "treat", "dose", "val"))
```
...
```{r fig.height = as.numeric(custom_fig_height), fig.width = custom_fig_width, eval = are_there_plates}
(plate_plot +
  geom_point(mapping = aes(fill = Gini_data, val = Gini_data), data = QAQC_annotated,
             shape = 21, size = 3, colour = "grey20", stroke = 0.5) +
  scale_threshold(QAQC_annotated$Gini_data,
                  threshold = params$gini_cutoff,
                  aesthetics = "fill",
                  low = "#2166AC", high = "#B2182B",  # high Gini is the failing direction
                  name = "Gini",
                  breaks = c(min(QAQC_annotated$Gini_data), params$gini_cutoff,
                             max(QAQC_annotated$Gini_data))) +
  ggtitle("Gini")) %>%
  ggplotly(tooltip = c("name", "treat", "dose", "val"))
```
...
## Dendrogram, all samples {.tabset}

Technical control samples are colored differently.

### Pre-filtering
```{r dendrogram_all_before}
...
## Dendrogram, technical controls 

Colored by `r params$dendro_color_by`.

Note that technical controls are filtered out, so no post-filtering dendrogram is available.
...
## Dendrogram, experimental samples {.tabset}
...
### Prefiltering, coloured by group
...
### Prefiltering, coloured by dose

```{r dendro_samples_before_dose}
...
### Postfiltering, coloured by group
...
### Postfiltering, coloured by dose
...
## Group-based Dendrogram Filtering  {.tabset}

If you set a group tree height cutoff in the config file, then any sample separating from the rest of its group at a height heigher than `r params$group_tree_height_cutoff` will be filtered out.
...
These plots show the effects of batch and dose on the quality metrics used for filtering.
...
If your experiment used the DRUG-seq method, quality control metrics that are only applicable to DRUG-seq data will appear here.

## Proportion of ERCC spike-in reads

If you included [ERCC spike-in RNA](https://www.thermofisher.com/order/catalog/product/4456740) , reads matching the ERCC sequences should represent 3-5% of the obtained mapped reads.

The green bar in the ERCC plot represents the 3-5% range.

Higher proportions of ERCC reads in a sample suggest that there was not as much input RNA as expected.

Note that these plots are made using the data from count_table.tsv, and reflect the UMI deduplication method used to produce that count table.
In this project, that UMI deduplication method was `r {params$umi_method_definitive}`.

```{r ercc_stats, eval=isTRUE(params$platform == "DRUG-Seq"), fig.height = as.numeric(custom_fig_height)*1.5}
# Ideally this would evaluation only if params$include_ercc is TRUE,
# But params$include_ercc isn't part of the loaded params

# Calculate and plot the proportion of reads that match ERCC spike-in sequences
count_ercc <- count_data %>%
  rownames_to_column("gene") %>%
  filter(grepl("ERCC", gene)) %>%
  column_to_rownames("gene")

ercc_sums <- colSums(count_ercc)
total_sums <- colSums(count_data)
...
ercc_stats <- tibble(
  ...
  dplyr::left_join(exp_metadata, by = "sample_ID") #add metadata to get library_ID for faceting

# Output table
write.table(ercc_stats, file.path(paths$details, "ercc_stats.txt"), quote = FALSE, sep = "\t", row.names = FALSE)
# The figure height should probably be improved

ercc_plot <- ggplot(ercc_stats, aes(y = .data[[params$treatment_var]], color = .data[[params$treatment_var]])) +
  annotate(
    "rect",
    xmin = 0.03, xmax = 0.05,
  ...
  geom_boxplot(aes(x = ercc_proportion)) +
  # geom_point(aes(x = ercc_proportion)) +
  facet_wrap(~library_ID, 
             ncol = 1,
             scales = "free_y") +
  ggtitle("Proportion of mapped reads that map to ERCC spike-in RNA")

ercc_plot
```

## UMI Deduplication
This plot shows the percent of sequences in each sample that were unique. This is calculated as the sum of reads in the deduplicated count table over the sum of reads in a non-deduplicated count table for each sample.

Percent unique read values lower than approximately 70% suggest that there may not have been sufficient RNA input, or there were other problems prior to sequencing.



```{r dedup_figures,  eval=isTRUE(params$platform == "DRUG-Seq"), fig.height = as.numeric(custom_fig_height)*1.5}
# Library IDs for faceting
libraries <- exp_metadata$library_ID %>% unique()

extract_library <- function(file_string, lib_list) {
  for (lib in lib_list) {
    if (grepl(lib, file_string)) {
      return(lib)
    }
  }
  return("NA")
...
# Import deduplication data from files made by rule quantify_dedup
dedup_files <- list.files(path = paste0(paths$qc, "/drugseqQC/"), pattern = "countsums.txt$", full.names = TRUE)

dedup_tables <- lapply(dedup_files, read.table)
names(dedup_tables) <- tools::file_path_sans_ext(basename(dedup_files))
dedup_tables <- lapply(dedup_tables, rownames_to_column, var = "sample")

# Add a column with the file name (contains library and UMI method) to each dataframe
dedup_tables <- lapply(names(dedup_tables), function(name) {
  dedup_tables[[name]] %>%
    mutate(file = name)
})

# Merge all tables into one big dataframe
dedup_df <- bind_rows(dedup_tables)

# Create new columns by pattern matching to get library and UMI method info
dedup_df <- dedup_df %>%
  mutate(
    umiMethod = case_when(
      grepl("umi1MM_All", file) ~ "All",
      grepl("umi1MM_CR", file) ~ "CR",
      grepl("umi1MM_Directional_dedup", file) ~ "Directional",
      grepl("umi1MM_Directional_UMItools", file) ~ "Directional_UMItools",
      grepl("umiExact", file) ~ "Exact",
      TRUE ~ "NA"
    )
  ) %>%
  mutate(
    # Use helper function from above to get library name from filename
    Library = sapply(file, function(x) extract_library(x, libraries))
  ) %>%
  dplyr::rename(sample_ID = "sample")

# Add grouping info from metadata then plot
exp_metadata %>% dplyr::select(c("sample_ID", params$treatment_var)) %>%
  full_join(dedup_df) %>%
  ggplot(aes(y = .data[[params$treatment_var]])) +
    annotate(
    "rect",
    xmin = 0.7, xmax = 1.0,
    ymin = -Inf, ymax = Inf,  # Full vertical range
    ...
    geom_boxplot(aes(x = ratio, color = umiMethod)) +
    facet_wrap(~Library, 
               scales = "free_x", 
               ncol = 1) +
    xlim(0,1) +
    labs(x = "Percent unique reads (dedup ratio)",)
...
```{r correlations_group, fig.width = 9, fig.height = 8, results='asis'}
...
```{r correlations_group2,  fig.width = 9, fig.height = 8}
...
```{r correlations_group3,  fig.width = 9, fig.height = 8}
# MEANS BY CHEMICAL
...
if (length(exp_groups) > 1 && !is.na(params$batch_var)) {
  for (i in seq_along(exp_groups)[seq_along(exp_groups)]) {
