...
paths$metadata <- paste0(paths$data, "/metadata")
paths$reports <- paste0(paths$root, "/reports")
paths$qc <- paste0(paths$root, "/analysis/QC")
...
if (Platform == "TempO-Seq") {
    ...
    MappedUnmapped <- file.path(paths$processed,"mapped_unmapped.csv")
    sampledata_sep <- ","
    ...
    if (params$collapse_replicates == T) {
      ...
      write.table(sampleData,
                  file = file.path(paths$processed,"count_table_collapsed.csv"),
                  sep = ",",
                  quote = F,
                  row.names = F)
      mu <- collapse_reps(MappedUnmapped)
      write.table(mu,
                  file = file.path(paths$processed,"mapped_unmapped_collapsed.csv"),
                  sep = ",",
                  quote = F,
                  row.names = F)
    } else {
...
if (Platform == "TempO-Seq") {
  FMR_df <- rownames_to_column(mu) %>% mutate(FMR = pct_mapped) %>% select(rowname,FMR)
...
```{r possible_covariates, fig.height = 12, fig.width = 12, eval = !is.null(batch_var)}
...
plate_plot <- ggplot(QAQC_annotated, aes(x = factor(!!ensym(colname)),
  ...
  coord_fixed()  +
  {if (!is.null(batch_var))  facet_wrap(paste0("~",batch_var), ncol = 2)}
#  {if (!is.null(batch_var))  facet_rep_wrap(paste0("~",batch_var), repeat.tick.labels = T)}
...
custom_fig_height <- ifelse(test = !is.null(batch_length), yes = batch_length, no = 7)
```
...
(plate_plot +
  ...
  ggplotly(tooltip = c("name","treat","dose","val")) # Add x and y for row and column... not really necessary though
```
...
```{r plot_batch_effects, eval = !is.null(batch_var), fig.height = as.numeric(custom_fig_height)/2}
...
plot_batches <- ggplot(QAQC_annotated, aes(x = !!ensym(batch_var), color = !!ensym(treatment_var))) +
  facet_wrap(paste0("~",batch_var), scales = "free_x", labeller = labeller(
    .cols = add_spaces,
    .default = label_wrap_gen(20)), ncol = 4) +
...
```{r plot_chemical_differences, eval=any(grepl("dose", colnames(QAQC_annotated), ignore.case = T)), fig.height = as.numeric(custom_fig_height)/2}
plot_dose_effect <- ggplot(QAQC_annotated, aes(x = !!ensym(treatment_var), color = factor(!!ensym(dose)))) +
