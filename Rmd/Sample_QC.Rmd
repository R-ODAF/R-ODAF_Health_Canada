...
params:
  ...
  dendro_color_by: "Treatment" # Specify how you would like to color the dendrograms
  ...
  groups:
    value:
      one: ["Description", "Dose", "Treatment"]
...
In other words, for each sample, what is its mean correlation with every sample **within in its epxerimental grouping**?
...
colnum <- which(grepl("column", colnames(QAQC_annotated), ignore.case = T))
rownum <- which(grepl("row", colnames(QAQC_annotated), ignore.case = T))
...
(plate_plot +
  ...
  {if (!is.null(batch_var)) facet_wrap(paste0("~",batch_var))}) %>%
...
knitr::kable(QAQC_annotated %>%
               dplyr::group_by(!!ensym(treatment_var), !!ensym(dose), Any) %>%
...
plot_batches <- ggplot(QAQC_annotated, aes(x = !!ensym(batch_var), color = !!ensym(treatment_var))) +
  facet_grid(paste0("~",batch_var), scales = "free_x", labeller = labeller(.cols = label_wrap_gen(10))) +
