...
summarise_degs <- function(bootstrapped_list) {
  degs <- lapply(bootstrapped_list, function(x) row.names(as.data.frame(x$DESeqResDEGs)))
  degs_length <- unlist(lapply(degs, length))
  #degs_df <- do.call(rbind, degs) # problematic for empty data
  #degs_df <- dplyr::bind_rows(degs, .id = "df_id")
  ...
  df <- df %>%
    dplyr::group_by(n_per_group, range) %>%
    add_count(name = "binned_counts")
  df$y <- 1
  return(df)
}

tk6df <- summarise_degs(tk6)
tk6df$cell_line <- "TK6"

spheroidsdf <- summarise_degs(spheroids)
spheroidsdf$cell_line <- "Liver spheroids"

hepargdf <- summarise_degs(heparg)
hepargdf$cell_line <- "HepaRG"

mcf7df <- summarise_degs(mcf7)
mcf7df$cell_line <- "MCF-7"

all_cell_lines <- dplyr::bind_rows(spheroidsdf, tk6df ) #, hepargdf, , mcf7df) 

ggplot(data = all_cell_lines, aes(x=n_per_group, y=y))+
