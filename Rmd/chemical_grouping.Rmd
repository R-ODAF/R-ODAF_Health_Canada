...
### Load summarize_across_facets info
...
## pvclust package/function - similarity calculations

## PLSDA? Partial least squares-discriminant analysis

## Cosine similarity:

https://stackoverflow.com/questions/67352150/writing-an-r-function-to-find-cosine-similarity
https://stackoverflow.com/questions/2535234/find-cosine-similarity-between-two-arrays
https://github.com/cran/lsa/blob/master/R/cosine.R
https://stats.stackexchange.com/questions/31565/compute-a-cosine-dissimilarity-matrix-in-r
https://www.geeksforgeeks.org/how-to-calculate-cosine-similarity-in-r/
...
matrix_biomarker_genes <- merge(matrix_biomarker_genes,
                                er_biomarker  %>% dplyr::select(Symbol, Fold.Change),
                                by.x = "gene", by.y = "Symbol")
# rename Fold.Change column to "ER_biomarker"
matrix_biomarker_genes <- matrix_biomarker_genes %>%
  dplyr::rename(ER_biomarker = Fold.Change)
id <- matrix_biomarker_genes$gene

matrix_biomarker_genes <- as.matrix(matrix_biomarker_genes[, -1])
rownames(matrix_biomarker_genes) <- id
matrix_biomarker_genes[is.na(matrix_biomarker_genes)] <- 0

# pick columns from matrix_biomarker_genes based on bioactivity > 1% and bioactivity > 2%

matrix_biomarker_genes <- matrix_biomarker_genes[, c(colnames(matrix_DEGs_bioactivity_gt_0.01),"ER_biomarker")]

hca_biomarker <- hclust( as.dist(1-cor(as.matrix(matrix_biomarker_genes),
                                  method = dist_method) ),
                         method = link_method)
...
e <- prcomp(t(matrix_biomarker_genes), retx = TRUE, center = TRUE, scale. = TRUE)
# plot it
...
# REDO WITH SCALED LOG2 FOLD CHANGES

```{r }

matrix_DEGs_all_scaled <- scale(matrix_DEGs_all, center=FALSE, scale=colSums(matrix_DEGs_all))
...
# Originally...
# hclust(dist(t(matrix_DEGs_all), method = dist_method ), method = link_method)
clustered_DEGs_all_scaled <- hclust( as.dist(1-cor(as.matrix(matrix_DEGs_all_scaled),
                                  method = dist_method)),
                              method = link_method)

#dend_all <- as.dendrogram(clustered_DEGs_all)
dend_scaled <- as.dendrogram(clustered_DEGs_all_scaled)
# Extract unique chemicals and assign colorblind-friendly palette
unique_chemicals_scaled <- unique(sapply(colnames(matrix_DEGs_all_scaled), function(label) {
  parts <- unlist(strsplit(label, "_"))
  parts[1]  # Extract the chemical part
}))

# Generate a color palette with a different color for each unique chemical
color_palette_scaled <- viridis(length(unique_chemicals_scaled))

# Create a named vector mapping chemicals to colors
chemical_colors <- setNames(color_palette_scaled, unique_chemicals_scaled)

# Customize dendrogram labels
labels_colors_scaled <- sapply(labels(dend_scaled), function(label) {
  parts <- unlist(strsplit(label, "_"))
  chemical <- parts[1]
  color <- chemical_colors[chemical]
  list(text = paste(chemical, parts[2], sep = "_"), col = color)
...
# Set label attributes
labels(dend_scaled) <-  gsub("_vs_DMSO.*", "", labels(dend_scaled))
# Adjust text size
...
par(mar = c(20, 4, 4, 4), cex = 0.5)
plot(dend_scaled, horiz = F, main = "All doses, scaled")
...
matrix_DEGs_top_bioactive_0.1_scaled <- scale(matrix_DEGs_top_bioactive_0.1, center=FALSE, scale=colSums(matrix_DEGs_top_bioactive_0.1 ))


matrix_DEGs_top_bioactive_0.1_scaled <- hclust( as.dist(1-cor(as.matrix(matrix_DEGs_top_bioactive_0.1_scaled),
                                  method = dist_method) ),
                         method = link_method)
dend_top_bioactive_0.1_scaled <- as.dendrogram(matrix_DEGs_top_bioactive_0.1_scaled)

par(mar = c(20,4,4,4), cex = 0.75)
plot(dend_top_bioactive_0.1_scaled, horiz = F, main = "Top doses of bioactive chemicals with a 1% threshold, normalized log2 fold changes")
...
par(mar = c(20,4,4,4), cex = 0.75)
plot(dend.top.active_0.2, horiz = F, main = "Highest doses of bioactive chemicals with a 2% threshold, normalized log2 fold changes")
...
plotPCAcustom <- function(obj, trim_names = TRUE, ...) {
  df <- cbind(obj$x[,1:2]) %>% as.data.frame()
  pattern <- "^(.*?)(?=_vs_)"
  if (trim_names) {
    rownames(df) <- rownames(df) %>% stringr::str_extract(pattern)
...
plotPCAcustom(a) + ggtitle("Bioactivity > 1%, normalized log2 fold changes")

### Bioactivity > 2%  normalized log2 fold changes
b <- prcomp(t(matrix_DEGs_bioactivity_gt_0.02), retx = TRUE, center = TRUE, scale. = TRUE)
plotPCAcustom(b) + ggtitle("Bioactivity > 2%,  normalized log2 fold changes")


# PCA plots for inactive chemicals removed @ 1% bioactivity  normalized log2 fold changes
c <- prcomp(t(matrix_DEGs_top_bioactive_0.1), retx = TRUE, center = TRUE, scale. = TRUE)
plotPCAcustom(c) + ggtitle("inactive chemicals removed @ 1% bioactivity,  normalized log2 fold changes")


# PCA plots for inactive chemicals removed @ 2% bioactivity  normalized log2 fold changes
d <- prcomp(t(matrix_DEGs_top_bioactive_0.2), retx = TRUE, center = TRUE, scale. = TRUE)
plotPCAcustom(d) + ggtitle("inactive chemicals removed @ 2% bioactivity,  normalized log2 fold changes")
