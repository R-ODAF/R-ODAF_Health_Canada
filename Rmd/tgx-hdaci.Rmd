...
```{r process_data}
# Convert all columns except ID to numeric, collapse replicate IDs
df <- df %>%
...
my_pred <- data.frame(samples = names(merged)[-c(1:9)], probH = 0, probNH = 0)
for (k in 1:(ncol(merged) - 9)){
  #k <- 1
  H <- sum(((merged[, k + 9] - merged$HDACi.score) / merged$std.dev)^2) - 2 * log(0.5)
  NH <- sum(((merged[, k + 9] - merged$NonHDACi.score) / merged$std.dev)^2) - 2 * log(0.5)
