...
id_table_entrez <- params$biospyder %>%
  ...
  id_table_entrez <- AnnotationDbi::select(get(params$species_data$orgdb), columns = c("ENSEMBL", "SYMBOL", "ENTREZID"), keys = allResults$Ensembl_Gene_ID, keytype="ENSEMBL") %>% distinct()
...
if (nrow(matches) > 1) {
  ...
  if (nrow(DEG_pathways@result %>% filter(p.adjust < 0.01 & qvalue < 0.05)) > 1) {
    write.table(as.data.frame(DEG_pathways),
                file = file.path(paths$pathway_analysis[[params$display_group_filter]],"Enriched.WikiPathways.txt"))
  } else { cat("No significantly enriched terms using criteria selected") }
}
```
...
for (i in seq_along(contrasts) ){
  ...
  if (nrow(DEG_pathways@result %>% filter(p.adjust < 0.01 & qvalue < 0.05)) > 1) {
    plotList[[i]] <- clusterProfiler::dotplot(DEG_pathways, showCategory = 20, font.size = 9)  +
      ...
      scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 30))
    write.table(as.data.frame(DEG_pathways),
                file = file.path(paths$pathway_analysis[[params$display_group_filter]],paste0("Enriched.WikiPathways.",
...
for (i in seq_along(contrasts)) {
  ...
  write.table(as.data.frame(gsea_sorted),
              file = file.path(paths$pathway_analysis[[params$display_group_filter]],paste0("Enriched.KEGG_Pathways.",
                                                       str_replace_all(contrasts[i]," ","_"),
                                                       ".txt")))
