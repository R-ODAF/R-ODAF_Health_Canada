...
params:
  ...
  project_name: "Halappanavar CuO"        # Change depending on your project name
  species: "mouse"                 # one of human, mouse, rat, hamster
  design: "Treatment_dose_description"
  intgroup: ["Treatment_dose_description"]            # experimental group of interest plus covariates; can be more than one
  ...
  group_facet: Treatment  # If you have many different experimental groups, you may subset the report by specifying a column in the metadata to filter groups, and then setting the group of interest in group_filter
  group_filter:  Nano # Optional. Only look at a particular group within a facet.
  sortcol: Dose
...
if (species == "human") {
  ...
  if (species == "mouse") {
    ...
    kegg_organism <- "mmu"
    temposeq_manifest <- "181130 Mouse S1500+ Surrogate 1.2 Manifest.txt" # or 190603 Mouse Whole Transcriptome 1.1 Manifest.xlsx
} else {
  if (species == "rat") {
    # Rat: 
...
if (Platform == "RNA-Seq") {
  ...
  if (colnames(biospyder)[1] == "PROBE_NAME") {
    ...
    biomart_filter <- "Probe.Name"
    biospyder_filter = "external_gene_name"
  }
  		...
  		CountsPass <- NULL
  			for (group in 1:length(SampPerGroup)) { 
  			  # This line gets "sampleCols" from the DE_Design table.
  				...
  				sampleNamesGroup <- rownames(DE_Design[sampleCols,])
  ...
  print("Obtaining the DESeq2 results")
	...
	for (gene in 1:nrow(DECounts)) {
	  # Check the median against third quantile
		...
		sampleNames_g1 <- rownames(DE_Design[sampleColsg1,])
		sampleNames_g2 <- rownames(DE_Design[sampleColsg2,])
	  ...
	  Check <- median(as.numeric(DECounts[gene, sampleNames_g1])) > quantile(DECounts[gene, sampleNames_g2], 0.75)[[1]]
		...
		for (group in 1:length(SampPerGroup)) {
				...
				sampleNamesSpike <- rownames(DE_Design[sampleColsSpike, ])
...
if (Platform == "TempO-Seq") {
  genes <- biospyder[toupper(biospyder[[biomart_filter]]) %in% genes,] %>% pull(!!ensym(biospyder_ID))
  ...
  id_table_entrez$Probe.Name <- toupper(id_table_entrez$Probe.Name) #hackity hack hack for mouse S1500
