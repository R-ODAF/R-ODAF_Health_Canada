...
params:
  ...
  group_filter:  "CISP"       # null # Which group will this report be done on?
  ...
  biospyder_manifest_file: "191113_Human_S1500_Surrogate_2.0_Manifest.csv"
...
# summaryTable <- data.frame( genes = row.names(resListSig[[longest]]),
#                             baseMean = resListSig[[longest]]$baseMean )
...
## Filtered at `r metadata(resListSig[[1]])$alpha` {.tabset .tabset-fade}

This group of plots shows `alpha` = `r metadata(resListSig[[1]])$alpha`, which is the `alpha` value used to determine which resulting features were significant when running the function `DESeq2::results()`.  
