...
if (any(grepl(x = colnames(DESeqDesign), pattern = "dose", ignore.case = T))) {
  ...
  if(length(doseCol)>1){
    doseCol <- doseCol[1] # This should be assigned explictly instead. Todo.
  }
...
if (Platform == "TempO-Seq") {
  ...
  id_table_entrez[,biomart_filter] <- toupper(id_table_entrez[,biomart_filter]) #hackity hack hack for mouse S1500 - extra hack for cases where probe name isn't the column name
