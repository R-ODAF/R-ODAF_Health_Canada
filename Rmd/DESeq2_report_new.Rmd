...
allResultsOrdered_logFC_filter %>%
  ...
  geom_vline(xintercept = 0,
             ...
             color = "black",
             size = 1) +
  ggtitle(paste0("Top ",numResults," genes ranked by fold change (adjusted p-value <",params$alpha,"), grouped by treatment"))


```

# Gene-level plots for top `r params$nBestPlots` features

maybe we could cut this to 10, since we will have the data exploration plot. Maybe even make it a tabset? I do like having it myself because it gives granularity at the sample level - how much spread is in the data... are dose-response trends looking reasonable or not across doses...
```{r genelevel_plots}
...
plotCounts_gg <- function(i, d, intgroup) {
    plotdata <- plotCounts(d,
                       ...
                       intgroup = intgroup,
                       returnData = TRUE)
    plot_title <- paste(id_table[[params$biomart_filter]][grep(i,id_table[[params$biomart_filter]])],
                        id_table[["Ensembl_Gene_ID"]][grep(i, id_table[[params$biomart_filter]])])
    if (ncol(plotdata) > 2) {
      colorCol = 3
    } else {colorCol = 2}
