...
for(cont in volcano_contrasts[1:3]){
  for (status in all_status_keys){
    ...
    traces <- c(traces, paste0(cont,"_",status))
  }
}


vec_1 <- rep(FALSE, 3*length(all_status_keys))
vec_1[1:4] <- TRUE
vec_2 <- rep(FALSE, 3*length(all_status_keys))
vec_2[5:8] <- TRUE
vec_3 <- rep(FALSE, 3*length(all_status_keys))
vec_3[9:12] <- TRUE
list_1 <- as.list(c(FALSE, vec_1))
list_2 <- as.list(c(FALSE, vec_2))
list_3 <- as.list(c(FALSE, vec_3))
...
updatemenus <- list(
  list(
    ...
    buttons = list(
      list(method = "restyle",
           args = list("visible", list_1),
           label = volcano_contrasts[1]),
      
      list(method = "restyle",
           args = list("visible", list_2),
           label = volcano_contrasts[2]),
      
      list(method = "restyle",
           args = list("visible", list_3),
           label = volcano_contrasts[3])
    )
  )
)

fig <- fig %>% layout(title = "Highlight Clusters",
                      showlegend = TRUE,
