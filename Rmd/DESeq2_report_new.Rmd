...
if(params$platform == "TempO-Seq"){
  ...
  mart <- useMart(biomart = "ensembl", dataset = species_data$ensembl_species)
  
  id_table <- getBM(attributes = c("ensembl_gene_id","external_gene_name"), mart = mart) %>%
    dplyr::select(Feature_ID = ensembl_gene_id, Ensembl_Gene_ID = ensembl_gene_id, Gene_Symbol = external_gene_name)
...
Select a contrast from the dropdown list to filter by contrast.
...
plot_DEG_heatmap <- function(rld_data){
  ...
  if (nrow(as.matrix(mat)) > 1) {
    ...
    return(pheatmap(mat,
             annotation_col =  as.data.frame(colData(rld_DEGs)[, params$intgroup_to_plot, drop = F]),
