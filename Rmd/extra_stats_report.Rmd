...
allResults <- annotate_deseq_table(resultsList, params, bs, filter_results = F)
...
for (i in seq_along(resultsList)) {
  contrast = gsub(pattern = paste0("log2.*", params$design, "\ "),
                  replacement =  "",
                  x = resultsList[[i]]@elementMetadata[[2]][2])
  cat("###", contrast, "  \n\n")
  MA_title <- paste0('MA plot, alpha = ', metadata(resultsList[[i]])$alpha,', ', contrast)
  DESeq2::plotMA(resultsList[[i]],
                 alpha = metadata(resultsList[[i]])$alpha,
