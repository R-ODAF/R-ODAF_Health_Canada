...
# Get the data 
counts_by_group <- counts_by_group %>%
  dplyr::filter(gene %in% genesAlphabetized[[params$biomart_filter]])
...
top_gene = genesToSelect[[params$biomart_filter]][1]
...
bscols(widths = c(2,NA),
  list(
    filter_select("gene_to_filter",
                  ...
                  ~gene,
                  unique(genesAlphabetized[[params$biomart_filter]]),
                  multiple = F)
  ),
  list(
