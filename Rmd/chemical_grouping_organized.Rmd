...
This report is intended to provide a visual representation of the grouping of chemicals based on their gene expression profiles.
It uses data from a BPA and alternatives TempO-seq exposure experiment on MCF-7 cells. See the publication for this experiment [here](https://academic.oup.com/toxsci/article/191/2/266/6935810)
...
The distance method used throughout this report is `r dist_method` and the linkage method is `r link_method`.

Data used in this report is pulled from the RData object saved during the R-ODAF_Health_Canada workflow.
For more information on that pipeline, see the [R-ODAF_Health_Canada GitHub repository](https://github.com/R-ODAF/R-ODAF_Health_Canada/)
...
# So this never actually gets used....
if (include_estradiol == FALSE){
  skip_extra <- c("DMSO","water", "17B-estradiol") # Remove DMSO controls as a facet
} else
{
...
In this workflow, each chemical is considered a facet. The contrasts are the doses of each chemical vs. DMSO as a vehicle control.

The DESeq2 results are imported from the RData file as lists of dataframes. Each dataframe contains the results of a single contrast.
Gene symbol, ensembl ID, and a description have been added to each row of the DESeq2 results dataframe.

Here are the columns in the dataframes used for analysis:
 [1] "Gene_Symbol"      "Ensembl_Gene_ID"  "description"      "baseMean"        
 [5] "log2FoldChange"   "linearFoldChange" "lfcSE"            "pvalue"          
 [9] "padj"             "contrast"         "facet"  

```{r facet_data, echo=TRUE, results='hide', message=FALSE, warning=FALSE}
...
# Remove estradiol from dataset if indicated in settings up top
if (include_estradiol == FALSE){
  significantResultsUnfaceted <- significantResultsUnfaceted %>% filter(facet != "17B-estradiol")
  allResultsUnfaceted <- allResultsUnfaceted %>% filter(facet != "17B-estradiol")

  facets <- facets[facets != "17B-estradiol"]

  filtered_table <- filtered_table %>% filter(facet != "17B-estradiol")
}
```


To decide which doses to use for clustering, we look at the number of DEGs in each facet (chemical) and the total number of genes measured for that chemical.
This is plotted as a proportion: the number of DEGs divided by the total number of genes measured.

We decided to use a threshold of 0.01% of all genes measured as the minimum number of DEGs to consider a dose for clustering.
This removes biologically inactive chemicals and doses from the clustering analysis.

We end up with a list of contrasts of interest (ie bioactive chemicals and doses) that we will use for clustering.
```{r process_data}
...
ggplot(proportions, aes(x = facet_contrast, y = perc)) +
  geom_bar(stat="identity", position="dodge") +
  geom_hline(yintercept = 0.01, linetype = "dashed", color = "red", linewidth = 1.5) +
  facet_wrap(~facet, scales = "free_x") +
    theme_bw() +
    theme(axis.text.x = element_blank()) 
...
Hierarchical clustering of all doses of all chemicals, based on the log2 fold changes of DEGs.
...
if (include_estradiol == FALSE){
  results_selected_all_processed <- results_selected_all_processed %>%
    dplyr::filter(!str_detect(name, "17B-estradiol"))
}
...
Only the highest dose of each chemical (regarless of whether it passes the bioactivity threshold defined above.)
...
if (include_estradiol == FALSE){
  results_selected_processed_top <- results_selected_processed_top %>%
    dplyr::filter(!str_detect(name, "17B-estradiol"))
}
...
We then performed the same clustering analysis as above, but with the log2 fold changes scaled by the sum of the column.
...
Only the lowest dose of each chemical (regarless of whether it passes the bioactivity threshold defined above.)
...
if (include_estradiol == FALSE){
  results_selected_processed_low <- results_selected_processed_low %>%
    dplyr::filter(!str_detect(name, "17B-estradiol"))
}
...
Now we begin using the bioactivity threshold defined above to filter out inactive chemicals and doses.
The plot below includes only chemicals and doses with a bioactivity greater than 1%.
I'm actually not sure if it's 1% or 0.01%. I feel like we figured that out at some point.
...
# Remove the _vs_DMSO part of the name
results_selected_bioactivity_gt_0.01_filt <- results_selected_bioactivity_gt_0.01_filt %>%
  mutate(name = sub("_vs.*", "", name))
...
par(mar = c(6.5,4,4,4), cex = 1)
plot(as.dendrogram(clustered_DEGs), horiz = F, main = "Bioactivity DEGs >1%")

```

#### Clustering with pvclust instead of hclust
```{r pvclust_bioactive0.01}
pvresult_0.01 <- pvclust(matrix_DEGs_bioactivity_gt_0.01, method.dist="cor", method.hclust="average", nboot=10000, parallel=TRUE)

plot(pvresult_0.01)
pvrect(pvresult_0.01, alpha=0.95)
```


# Trying to find optimal grouping by 3 methods

We want to find the optimal number of clusters for the data. We will use three methods to do this: the elbow method, the silhouette method, and the gap statistic method.

## Sillouette method

```{r silhouette_bioactive0.01}
# SILHOUETTE METHOD FOR NUMBER OF CLUSTERS

# Determine the number of clusters (e.g., using the silhouette method)
library(cluster)
sil_width <- numeric(10)

# First have to convert the matrix of log2FCs into a similarity matrix
similarity_matrix <- cor(t(matrix_DEGs_bioactivity_gt_0.01), method = "pearson")

for (k in 2:10) {
  pam_fit <- pam(similarity_matrix, diss = FALSE, k = k) # Actually I think diss = TRUE. Need to check with Andrew.
...
clusters <- cutree(clustered_DEGs, k = optimal_clusters)
...
par(mar = c(20, 4, 4, 4), cex = 0.5)
plot(as.dendrogram(clustered_DEGs), horiz = FALSE, main = "Bioactivity DEGs >1%")
rect.hclust(clustered_DEGs, k = optimal_clusters, border = 2:optimal_clusters + 1)

```

## Gap statistic method

```{r gap_stat_bioactive0.01}
# GAP STATISTIC FOR NUMBER OF CLUSTERS

# Calculate the gap statistic
set.seed(123) # For reproducibility

# Provide the matrix of log2 fold changes, not a similarity or dissimilarity matrix. 
# The clusGap function will internally handle the clustering and calculation of the gap statistic.
gap_stat <- clusGap(matrix_DEGs_bioactivity_gt_0.01, 
                    FUN = function(x, k) list(cluster = cutree(hclust(as.dist(1 - cor(t(x), method = dist_method)), method = link_method), k = k)), 
                    K.max = 10, B = 50)
...
unique_clusters <- max(unique(clusters))
# When estradiol included, Plot fails because gap_stat says the optimal number of clusters is 1

if (unique_clusters > 1) {
  # Plot the dendrogram with clusters
  par(mar = c(6.5, 4, 4, 4), cex = 1)
  plot(as.dendrogram(clustered_DEGs), horiz = FALSE)
  rect.hclust(clustered_DEGs, k = optimal_clusters, border = 2:optimal_clusters + 1)
} else {
  message("This plot could not be printed because the optimal number of clusters is 1.")
}
...
```{r elbow_bioactive0.01}
# ELBOW METHOD FOR NUMBER OF CLUSTERS

#For the elbow method, you should also provide the original data matrix (matrix_DEGs_bioactivity_gt_0.01). The elbow method involves calculating the total within-cluster sum of squares (WSS) for different numbers of clusters and plotting these values to identify the "elbow" point where the rate of decrease sharply slows.

# Calculate the total within-cluster sum of squares (WSS) for different numbers of clusters
wss <- numeric(10)
for (k in 1:10) {
  wss[k] <- sum(kmeans(matrix_DEGs_bioactivity_gt_0.01, centers = k, nstart = 25)$tot.withinss)
}
...
optimal_clusters <- which(diff(diff(wss)) == min(diff(diff(wss)))) + 1
...
if (unique_clusters > 8) {
  message_text <- paste("This plot could not be printed because the optimal number of cluster (", unique_clusters, ") exceeds the allowed number.")
...
Unfortunately, the three methods tried above do not agree on the optimal number of clusters.
Andrew suggests that this is because of the small sample size.


This is the same plot as above, but but with the log2 fold changes scaled by the sum of the column.
...
This is the same plot as above, but with log2FC scaled by the sum of the column.
...
#### Clustering with pvclust instead of hclust
```{r pvclust_bioactive0.02}
pvresult_0.02 <- pvclust(matrix_DEGs_bioactivity_gt_0.02, method.dist="cor", method.hclust="average", nboot=10000, parallel=TRUE)

plot(pvresult_0.02, cex=0.75)
pvrect(pvresult_0.02, alpha=0.95)
```

### Top doses with inactive chemicals removed @ 1% bioactivity

No scaling was done in these heirarchical clusters. 

```{r unscaled_top_doses_bioactive_0.1}
...
# Modify the column names to remove the _vs_DMSO part
colnames_matrix_DEGs_top_bioactive_0.1 <- colnames(matrix_DEGs_top_bioactive_0.1)
colnames_matrix_DEGs_top_bioactive_0.1 <- sub("_vs.*", "", colnames_matrix_DEGs_top_bioactive_0.1)
colnames(matrix_DEGs_top_bioactive_0.1) <- colnames_matrix_DEGs_top_bioactive_0.1
...
par(mar = c(8,4,4,4), cex = 1)
plot(dend.top.active_0.1, horiz = F, main = "Top doses of bioactive chemicals with a 1% threshold")
```

#### Clustering with pvclust instead of hclust
```{r pvclust_topdose_0.01}
pvresult_top_0.01 <- pvclust(matrix_DEGs_top_bioactive_0.1, method.dist="cor", method.hclust="average", nboot=10000, parallel=TRUE)

plot(pvresult_top_0.01)
pvrect(pvresult_top_0.01, alpha=0.95)
```
...
## Sillouette method
Sillouette method is not working on this one, it hangs forever on the for (k in 2:10) loop. Commented out the code.

```{r silhouette_topdose_0.01}
# SILHOUETTE METHOD FOR NUMBER OF CLUSTERS
...
# # First have to convert the matrix of log2FCs into a similarity matrix
# similarity_matrix <- cor(t(matrix_DEGs_top_bioactive_0.1), method = "pearson")

# for (k in 2:10) {
#   pam_fit <- pam(similarity_matrix, diss = FALSE, k = k)
#   sil_width[k] <- pam_fit$silinfo$avg.width
# }
# optimal_clusters <- which.max(sil_width)
...
# Provide the matrix of log2 fold changes, not a similarity or dissimilarity matrix. 
# The clusGap function will internally handle the clustering and calculation of the gap statistic.
gap_stat <- clusGap(matrix_DEGs_top_bioactive_0.1, 
                    FUN = function(x, k) list(cluster = cutree(hclust(as.dist(1 - cor(t(x), method = dist_method)), method = link_method), k = k)), 
                    K.max = 10, B = 50)
...
wss <- numeric(10)
for (k in 1:10) {
  wss[k] <- sum(kmeans(matrix_DEGs_top_bioactive_0.1, centers = k, nstart = 25)$tot.withinss)
}
...
if (unique_clusters > 7){
  message_text <- paste("This plot could not be printed because the optimal number of cluster (", unique_clusters, ") exceeds the allowed number.")
  message(message_text)
} else if (unique_clusters < 3) {
   message_text <- paste("This plot could not be printed because the optimal number of cluster (", unique_clusters, ") is too small.")
   message(message_text)
} else {
  # Plot the dendrogram with clusters
  par(mar = c(6.5, 4, 4, 4), cex = 1)
...
par(mar = c(16,4,4,4), cex = 1)
plot(dend.top.active_0.2, horiz = F, main = "Highest doses of bioactive chemicals with a 2% threshold")

```

#### Clustering with pvclust instead of hclust
```{r pvclust_topdose_0.02}
pvresult_top_0.02 <- pvclust(matrix_DEGs_top_bioactive_0.2, method.dist="cor", method.hclust="average", nboot=10000, parallel=TRUE)

plot(pvresult_top_0.02, cex=0.75)
pvrect(pvresult_top_0.02, alpha=0.95)
```
...
plotPCAcustom <- function(obj, title = "", trim_names = FALSE, ...) {
  ...
  if (length(unique(df$V3)) > length(cbp1)) {
    cbp1 <- hcl.colors(length(unique(df$V3)), "Roma")
  }
...
rownames(matrix_DEGs_bioactivity_gt_0.01) <- make.unique(rownames(matrix_DEGs_bioactivity_gt_0.01))
rownames(matrix_DEGs_bioactivity_gt_0.02) <- make.unique(rownames(matrix_DEGs_bioactivity_gt_0.02))
rownames(matrix_DEGs_top_bioactive_0.1) <- make.unique(rownames(matrix_DEGs_top_bioactive_0.1))
rownames(matrix_DEGs_top_bioactive_0.2) <- make.unique(rownames(matrix_DEGs_top_bioactive_0.2))
...
### ER biomarker genes
The ER50 biomarker is a set of 50 genes and their fold changes that are used to predict estrogen receptor activity.
In the following plots, we subset the DEGs to only include the ER biomarker genes.


First plot, with ER biomarker fold changes included.
...
# remove the _vs_DMSO part first so names match
colnames_matrix_biomarker_genes_with_er <- colnames(matrix_biomarker_genes_with_er)
colnames_matrix_biomarker_genes_with_er <- sub("_vs.*", "", colnames_matrix_biomarker_genes_with_er)
colnames(matrix_biomarker_genes_with_er) <- colnames_matrix_biomarker_genes_with_er
...
# plot dendrogram
par(mar = c(20,4,4,4), cex = 0.75)
plot(hca_biomarker_dendro, horiz = F, main = "ER biomarker genes + ER biomarker data")

# make a PCA of the matrix
e <- prcomp(t(matrix_biomarker_genes_with_er ), retx = TRUE, center = TRUE, scale. = TRUE)
# plot it
plotPCAcustom(e, trim_names = F, legend.position = "right", title = "ER biomarker genes + ER biomarker data")

```



HC and PCA of genes used in ER biomarker, but without including the ER biomarker itself.

```{r er_genes_no_er}
# pick columns from matrix_biomarker_genes based on bioactivity > 1% and bioactivity > 2%

# remove the _vs_DMSO part first so names match
colnames_matrix_biomarker_genes <- colnames(matrix_biomarker_genes)
colnames_matrix_biomarker_genes <- sub("_vs.*", "", colnames_matrix_biomarker_genes)
colnames(matrix_biomarker_genes) <- colnames_matrix_biomarker_genes

matrix_biomarker_genes_bioactive0.01  <- matrix_biomarker_genes [, c(colnames(matrix_DEGs_bioactivity_gt_0.01))]
matrix_biomarker_genes_bioactive0.01  <- as.matrix(matrix_biomarker_genes_bioactive0.01)
...
hca_biomarker_genes <- hclust( as.dist(1-cor(as.matrix(matrix_biomarker_genes_bioactive0.01),
                                  method = dist_method) ),
                         method = link_method)
hca_biomarker_genes_dendro <- as.dendrogram(hca_biomarker_genes)

# plot dendrogram
par(mar = c(6.5,4,4,4), cex = 1)
plot(hca_biomarker_genes_dendro, horiz = F, main = "ER-biomarker genes, bioactivity > 1%, ER biomarker data not included")

# make a PCA of the matrix
f <- prcomp(t(matrix_biomarker_genes_bioactive0.01), retx = TRUE, center = TRUE, scale. = TRUE)
# plot it
plotPCAcustom(f, trim_names = F, legend.position = "right", title = "ER-biomarker genes, Bioactivity > 1%, ER biomarker data not included")

```


#### Clustering with pvclust instead of hclust
```{r pvclust_erbiomarker}
pvresult_erbiomarker <- pvclust(matrix_biomarker_genes_bioactive0.01, method.dist="cor", method.hclust="average", nboot=10000, parallel=TRUE)

plot(pvresult_erbiomarker)
pvrect(pvresult_erbiomarker, alpha=0.95)
```
...
# First have to convert the matrix of log2FCs into a similarity matrix
similarity_matrix <- cor(t(matrix_biomarker_genes_bioactive0.01), method = "pearson")
...
# Provide the matrix of log2 fold changes, not a similarity or dissimilarity matrix. 
# The clusGap function will internally handle the clustering and calculation of the gap statistic.
gap_stat <- clusGap(matrix_biomarker_genes_bioactive0.01, 
                    FUN = function(x, k) list(cluster = cutree(hclust(as.dist(1 - cor(t(x), method = dist_method)), method = link_method), k = k)), 
                    K.max = 10, B = 50)
...
wss <- numeric(10)
for (k in 1:10) {
  wss[k] <- sum(kmeans(matrix_biomarker_genes_bioactive0.01, centers = k, nstart = 25)$tot.withinss)
}
...
```{r er_genes_PCA}
# pick columns from matrix_biomarker_genes based on top doses

matrix_biomarker_genes_topactive  <- matrix_biomarker_genes [, c(colnames(matrix_DEGs_top_bioactive_0.1))]
matrix_biomarker_genes_topactive  <- as.matrix(matrix_biomarker_genes_topactive)
rownames(matrix_biomarker_genes_topactive) <- id
matrix_biomarker_genes_topactive[is.na(matrix_biomarker_genes_topactive)] <- 0

# Hierarchical clustering
hca_biomarker_genes_topactive <- hclust( as.dist(1-cor(as.matrix(matrix_biomarker_genes_topactive),
                                  method = dist_method) ),
                         method = link_method)
hca_biomarker_genes_topactive_dendro <- as.dendrogram(hca_biomarker_genes_topactive)

# plot dendrogram
par(mar = c(8,4,4,4), cex = 1)
plot(hca_biomarker_genes_topactive_dendro, horiz = F, main = "ER-biomarker genes, top doses of active chems, ER biomarker data not included")

# make a PCA of the matrix
g <- prcomp(t(matrix_biomarker_genes_topactive), retx = TRUE, center = TRUE, scale. = TRUE)
# plot it
plotPCAcustom(g, trim_names = F, legend.position = "right", title = "ER-biomarker genes, top doses of active chems, ER biomarker data not included")

```

## Sillouette method

```{r silhouette_erbiomarkertop}
# SILHOUETTE METHOD FOR NUMBER OF CLUSTERS
...
# First have to convert the matrix of log2FCs into a similarity matrix
similarity_matrix <- cor(t(matrix_biomarker_genes_topactive), method = "pearson")
...
# Provide the matrix of log2 fold changes, not a similarity or dissimilarity matrix. 
# The clusGap function will internally handle the clustering and calculation of the gap statistic.
gap_stat <- clusGap(matrix_biomarker_genes_topactive, 
                    FUN = function(x, k) list(cluster = cutree(hclust(as.dist(1 - cor(t(x), method = dist_method)), method = link_method), k = k)), 
                    K.max = 10, B = 50)
...
wss <- numeric(10)
for (k in 1:10) {
  wss[k] <- sum(kmeans(matrix_biomarker_genes_topactive, centers = k, nstart = 25)$tot.withinss)
}
