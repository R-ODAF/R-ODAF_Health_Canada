...
wp2gene <- clusterProfiler::read.gmt(file.path(paths$wikipathways,wiki))
...
for (i in seq_along(levels(allResults$contrast))) {
  DEGs_full <- dplyr::left_join(allResults %>% filter(contrast ==  levels(allResults$contrast)[i]),
                                id_table_entrez,
                                by = biomart_filter)
  ...
  write.table(as.data.frame(gsea_sorted),
              file = file.path(paths$pathway_analysis,paste0("Enriched.KEGG_Pathways.",
                                                       str_replace_all(levels(allResults$contrast)[i],
                                                                       " ",
                                                                       "_"),
                                                       ".txt")))
...
for (i in seq_along(levels(allResults$contrast))) {
  if (any(!is.na(plotListGSEA[[i]]))) {
    cat("###", levels(allResults$contrast)[i], "  \n\n")
