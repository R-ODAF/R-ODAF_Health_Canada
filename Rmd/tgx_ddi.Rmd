...
samples <- colnames(log10ratios)
samples <- samples[-1] # Remove "ID"
...
t2$PCA <- ifelse(t1$PCA < -0.768, "DDI", 
                 ifelse(t1$PCA < 0, "Inconclusive", "Non-DDI"))

t2$NSC <- ifelse(t1$NSC > 0.9, "DDI",
                  ifelse(t1$NSC > 0.1, "Inconclusive", "Non-DDI"))
...
annotation_colors <- list(
  PCA = setNames(c("blue", "yellow", "red"), c("Non-DDI", "Inconclusive", "DDI")),
  HC = setNames(c("blue", "yellow", "red"), c("Non-DDI", "Inconclusive", "DDI")),
  NSC = setNames(c("blue", "yellow", "red"), c("Non-DDI", "Inconclusive", "DDI")),
  Overall = setNames(c("blue", "yellow", "red"), c("Non-DDI", "Inconclusive", "DDI"))
)

# Column order
pca_order <- c(DDI_chems, NDDI_chems, samples)

std_win_log10 <- std_win_log10[, pca_order]

# Reorder the rows in the annotation to match the matrix
t2 <- t2[match(pca_order, rownames(t2)), ]
...
```{r summary}
summary_table <- t2 %>% filter(row.names(t2) %in% samples)

# Color table cells
color_cells <- function(x) {
  ifelse(x == "DDI", cell_spec(x, "html", color = "red"), cell_spec(x, "html", color = "blue"))
}
summary_table$Overall <- sapply(summary_table$Overall, color_cells)

# Print the formatted table
kable(summary_table, caption = "Summary Table", escape = FALSE) %>%
  kable_styling("striped", full_width = FALSE)

summary_table$ID <- rownames(summary_table)
summary_table <- summary_table %>% select(ID, everything())
# Output the summary table to a file
# Remove HTML tags from the 'Overall' column
summary_table$Overall <- gsub("<.*?>", "", summary_table$Overall)

write.csv(summary_table, file = file.path(paths$reports_dir, paste0(params$reports_filter, "_tgx-ddi_results.csv")), row.names = FALSE)
```
