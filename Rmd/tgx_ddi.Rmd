...
dendro_biomarker <- data.frame(ID = names(merged_log2[,2:30]),
                                 HC = ifelse(names(merged_log2[,2:30]) %in% DDI_chems, "DDI", "Non-DDI"),
...
for (j in 31:ncol(merged_log2)){
  ...
  dendro_results_testchems <- rbind(dendro_results_testchems, data.frame(ID = colnames(merged_log2)[j], HC = result, stringsAsFactors = FALSE))
}
...
p$annotation_legend$PCA$legend_title <- NULL
p$annotation_legend$HC$legend_title <- NULL
p$annotation_legend$NSC$legend_title <- NULL
p$annotation_legend$Overall$legend_title <- NULL
...
summary_table2$HC <- ifelse(summary_table1$HC)
