...
pc1 <- df_pca[30:nrow(df_pca), ] %>% select(PC1) # Save PC1 for summary table
pc1$ID <- rownames(pc1)
rownames(pc1) <- NULL
...
summary_table2$PCA <- cut(summary_table1$PCA,
                          c(-Inf, 0, Inf),
                          labels = c("DDI", "Non-DDI"), right = FALSE)
...
kable(summary_table2, caption = "Summary Table")
```