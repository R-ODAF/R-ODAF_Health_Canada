...
# Fix annotations depending on temposeq kit
# Need to talk to Andrew to find additional fixes
...
if (params$biospyder_manifest_file == "Human_S1500_Surrogate_2.0_standardized.csv") {
  classifier <- classifier %>%
    mutate(ID = case_when(
      ID == "C12orf5" ~ "TIGAR",
      ID == "FAM123B" ~ "AMER1",
...
write.csv(summary_table2, file = file.path(paths$reports_dir, paste0(params$prefix, "_tgx-ddi_results.csv")), row.names = FALSE)
```
