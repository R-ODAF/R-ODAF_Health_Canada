...
---
title: "`r params$platform` TGx-DDI biomarker report: `r facet`"
---
...
# Currently hard-coded, will be different when running from render_DESeq2_report.parallel.R
#exp_contrasts <- R.ODAF.utils::get_contrasts("/home/bradford/storage/Pipeline_updates/Module_tgx-ddi/R-ODAF_Health_Canada/inputs/contrasts/contrasts.txt")
message(paste("Facet is", facet))

control <- exp_contrasts %>%
    filter(grepl(facet, V1)) %>%
    slice(1) %>%
    pull(V2)
...
```{r import_data}
df <- as.data.frame(allBiomarkers[[facet]])
...
# Log2 ratios vs DMSO_0
# NOTE: I'm going to need to generalize, or take in an argument about the control
# Could get this from the contrast info in Rdata?
log2ratios <- df_means
...
# This will need to be included in the repo
# Maybe in a "resources" folder?
classifier <- read.delim("/home/bradford/storage/Pipeline_updates/Module_tgx-ddi/TgxDDI_code/Biomarker_TGx-DDI_Li_2015_Tk6_cells_with_PCA_and_training_set.txt", sep = "")
