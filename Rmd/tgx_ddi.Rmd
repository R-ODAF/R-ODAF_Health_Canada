...
The test chemical is considered DDI if it is positive in at least one of these analyses.
...
#message(paste("Facet is", params$reports_filter)) # Message doesn't work if reports_filter is NULL (case 1)
...
# The sign of principle components is arbitrary, so we need to know the sign of the DDI classifier chemicals
# To figure out whether test chemicals are DDI or non-DDI
ddi_pc1 <- df_pca[1:13, ] %>% select(PC1)  # Save PC1 of DDI classifier chemicals
mean_ddi_sign <- sign(mean(ddi_pc1$PC1)) # Calculate the sign of the mean of the PC1 values for the DDI chemicals
nddi_pc1 <- df_pca[14:29, ] %>% select(PC1)  # Save PC1 of non-DDI classifier chemicals

pc1 <- df_pca[30:nrow(df_pca), ] %>% select(PC1) # Save PC1 of test chemicals for summary table
pc1$ID <- rownames(pc1)
rownames(pc1) <- NULL
pc1 <- pc1 %>% select(ID, everything())

# Set up labels and colours
df_pca$color <- factor(c(rep("DDI", 13), rep("non-DDI", 16), rep("tested", nrow(df_pca) - 29)))
df_pca$labels <- colnames(merged_log2[,-1])

# Create a scatter plot of the first two principal components with labels and colors
p <- ggplot(df_pca, aes(PC1, PC2, color = color)) +
  geom_point(aes(shape = color, text = paste("ID:", labels)), size = 2) +  
  scale_shape_manual(values = c("DDI" = 19, "non-DDI" = 19, "tested" = 18)) +  
  scale_color_manual(values = c("DDI" = "red", "non-DDI" = "blue", "tested" = "darkorange")) +
  labs(color = "Group") +
  theme_bw()

ggplotly(p, tooltip = "text")  # Hover text shows chemical names
...
dendro_results <- data.frame(ID = character(), dendro_DDI = character(), stringsAsFactors = FALSE)
...
for (j in 31:ncol(merged_log2)){
  ...
  plot(dendro, horiz = FALSE, main = paste("Euclidean, Average Linkage (log2),", colnames(merged_log2)[j]))
  cat("\n\n")
  ...
  dendro_results <- rbind(dendro_results, data.frame(ID = colnames(merged_log2)[j], dendro_DDI = result, stringsAsFactors = FALSE))
}
...
annotation_colors <- list(Prediction = c("Non-DDI" = "blue", "DDI" = "red"))
...
summary_table1 <- pc1 %>%
  merge(dendro_results, by = "ID") %>%
  merge(pred, by = "ID") %>%
  rename(PCA = PC1, HC = dendro_DDI, NSC = probG)

# Initialize
summary_table2 <- summary_table1

# Assign "DDI" or "Non-DDI" based on the comparison of the signs (because the sign of the PCA is arbitrary)
summary_table2$PCA <- ifelse(sign(summary_table1$PCA) == mean_ddi_sign, "DDI", "Non-DDI")

summary_table2$HC <- ifelse(summary_table1$HC == TRUE, "DDI",
                            ifelse(summary_table1$HC == "UNCERTAIN", "Inconclusive", "Non-DDI"))

summary_table2$NSC <- ifelse(summary_table1$NSC == 1, "DDI", "Non-DDI")

summary_table2$Result <- apply(summary_table2[, c("PCA", "HC", "NSC")], 1, function(x) ifelse("DDI" %in% x, "DDI", "Non-DDI"))
...
summary_table2$Result <- sapply(summary_table2$Result, color_cells)
