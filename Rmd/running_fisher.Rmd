...
# Should un-hardcode this, or write conditional so sentences below only show biomarkers actually used.
threshold4 <- c("AHR", "AR",
                "ERa50", "ERa46",
                "HSF1", "MTF1",
                "Nrf2-mouse", "TGx-DDI",
...
```{r complex_heatmap, echo=FALSE, fig.height=heatmap_height}
selected_data <- out2 %>%
  select(-Biomarker) %>%
  as.matrix() %>%
  t()
...
heatmap <- ComplexHeatmap::Heatmap(selected_data,
                   name = "-log10(p-value)",
