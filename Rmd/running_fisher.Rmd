...
# Currently hard-coding this in, but these will EITHER come from
## Biosets files from the results_location_arg OR
## biomarker files (same as TGX-DDI or HDACi), then reformatted to be a list of genes and fold changes
biosets <- dir(path = here::here("output/analysis/analysis_facetchemical_20240510-1219/BMD_and_biomarker_files"),
                      full.names = FALSE, 
                      recursive = FALSE)
biosets <- biosets[!grepl("_input_", biosets)]

bs <- NULL
indx <- 1

# Again, hard-coding right now but that will change!
# I just need to know how the biosets stuff looks with Andrew's code
# So I can find a way to get things in the same format after very different importing
for(k in 1:length(biosets)){
	files <- dir(paste(here::here("output/analysis/analysis_facetchemical_20240510-1219/BMD_and_biomarker_files"), "/", biosets[k], sep = "")) # looking for all the files in the directory (one directory per loop)
	if(length(files) > 0){
		for(f in 1:length(files)){	
			file <- files[f]

			tmp <- paste(here::here("output/analysis/analysis_facetchemical_20240510-1219/BMD_and_biomarker_files"), "/", biosets[k], "/", file, sep = "")
			tmp <- read.delim(tmp)
...
```{r plot_combined, echo=FALSE, fig.height=16, fig.width=10}
combined_plot
```