...
# To make heatmaps with categories of over/under threshold values
make_heatmap <- function (filtered_data, threshold_lo, threshold_hi, colours, breaks = NULL) {
  ...
  if (threshold_hi == 4) {
    ...
    title <- "Scores for Biomarkers with Threshold -4 to 4"
    fill = c("blue", "white", "red")
  } else if (threshold_hi == 15) {
    legend_text <- c(paste0("< ", threshold_lo), paste0(threshold_lo, " to ", threshold_hi), paste0("> ", threshold_hi))
    title <- "Scores for Biomarkers with Threshold -15 to 15"
    fill = c("blue", "white", "red")
  } 
  
  # Create the heatmap using the image function
  par(mar = c(2, 10, 16, 0), cex.main = 0.8)  # Margins: bottom, left, top, right
  ...
  par(mar = c(5, 0, 16, 0))  # Adjust margins for the legend
  plot.new()
...
```{r pheatmap, echo=FALSE, fig.height=heatmap_height}
...
# Set up colours and breaks so zero is white
paletteLength <- 21
...
myBreaks <- seq(-cont_legend_limit, cont_legend_limit, length.out = paletteLength + 1) %>% round()


# Plot heatmap of all biomarkers, continuous scale
pheatmap(as.matrix(selected_data),
         cluster_rows = FALSE,
         cluster_cols = FALSE,
         show_rownames = TRUE,
         show_colnames = TRUE,
         ...
         breaks = myBreaks,
         color = myColor,
         title = "Heatmap for All Biomarkers")
```
