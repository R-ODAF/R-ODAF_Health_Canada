...
# Apply cell_spec to color the background of columns 4 onward based on their values
final_df_colored <- final_df %>%
  mutate(across(4:ncol(final_df), ~ kableExtra::cell_spec(.x, "html", 
    color = "white",
    background = scales::col_numeric(
      palette = viridisLite::viridis(256, alpha = 0.3),
      domain = NULL
    )(.x),
    extra_css = "padding: 0px 4px; border-radius: 0px;")))

# Display the data frame using kable and kableExtra for formatting
knitr::kable(final_df_colored, escape = FALSE, caption = "Number of Genes and Matching Genes Between Biomarkers and Biosets") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = FALSE, position = "left") %>%
  kableExtra::add_header_above(c(" " = 3, "Number of matching genes" = length(bs))) %>%
  kableExtra::column_spec(1, bold = TRUE) %>%
  kableExtra::column_spec(2, bold = TRUE) %>%
  kableExtra::column_spec(3, italic = TRUE) %>%
  kableExtra::scroll_box(width = "100%", height = "700px", fixed_thead = TRUE)
```
...
```{r heatmap_threshold15, echo = FALSE}
# Filter the data
...
rownames(data_matrix_threshold15) <- selected_data_threshold15$Compounds

# Create the heatmap
pheatmap(data_matrix_threshold15,
         cluster_rows = FALSE,
         cluster_cols = FALSE,
         show_rownames = TRUE,
         show_colnames = TRUE,
         ...
         cellwidth = 10,
         cellheight = 10,
         main = "Heatmap for Biomarkers with Threshold -15 to 5",
         color = custom_colors,
         breaks = c(-20, -15, 15, 20),
         legend_breaks = c(-Inf, 0, Inf),
         legend_labels = c("< -15", "-15 to 15", "> 15"))
```

```{r heatmap_threshold4, echo=FALSE}
# Filter the data
...
# Set the row names of the matrix to the values in the 'Compounds' column
rownames(data_matrix_threshold4) <- selected_data_threshold4$Compounds

# Create the heatmap
pheatmap(data_matrix_threshold4,
         cluster_rows = FALSE,
         cluster_cols = FALSE,
         show_rownames = TRUE,
         show_colnames = TRUE,
         ...
         legend_breaks = c(-Inf, -4, 4, Inf),
         legend_labels = c("< -4", "-4 to 0", "0 to 4", "> 4"))
```

```{r heatmap_thresholdunkown, echo=FALSE}
