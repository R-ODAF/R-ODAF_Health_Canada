...
# Check for any NA values in the Gene, pval, or fc columns
# for (i in 1:length(bs)) {
#   if (any(is.na(bs[[i]]$Gene))) {
#     print(paste("NA in Gene column in bioset:", names(bs)[i]))
#   }
#   if (any(is.na(bs[[i]]$pval))) {
#     print(paste("NA in pval column in bioset:", names(bs)[i]))
#   }
#   if (any(is.na(bs[[i]]$fc))) {
#     print(paste("NA in fc column in bioset:", names(bs)[i]))
#   }
# }
...
custom_colors <- c("blue", "white", "red") # For heatmaps
```
...
selected_data_threshold15 <- out2 %>% 
  filter(Compounds %in% threshold15)

# Exclude the 'Compounds' column and convert the remaining data frame to a matrix
data_matrix_threshold15 <- selected_data_threshold15 %>%
  select(-Compounds) %>%
  as.matrix() %>%
  t()  # Transpose so biomarkers are on x-axis, biosets on y-axis

colnames(data_matrix_threshold15) <- selected_data_threshold15$Compounds

# Set up the layout: 1 row, 2 columns, so legend is beside the heatmap
layout(matrix(c(1, 2), nrow = 1), widths = c(4, 1))

# Create the heatmap using the image function
par(mar = c(2, 10, 10, 0), cex.main = 0.8)  # Margins: bottom, left, top, right
image(1:ncol(data_matrix_threshold15), 1:nrow(data_matrix_threshold15), t(data_matrix_threshold15),
      col = custom_colors, breaks = c(-100, -15, 15, 100), axes = FALSE, xlab = "", ylab = "",
      main = "Heatmap for Biomarkers with Threshold -15 to 15")

# Add row and column labels
axis(3, at = 1:ncol(data_matrix_threshold15), labels = colnames(data_matrix_threshold15), las = 2)
axis(2, at = 1:nrow(data_matrix_threshold15), labels = rownames(data_matrix_threshold15), las = 2)

# Add borders around the cells
for (i in 1:ncol(data_matrix_threshold15)) {
  for (j in 1:nrow(data_matrix_threshold15)) {
...
plot.new()
legend("center", legend = c("< -15", "-15 to 15", "> 5"), 
...
selected_data_threshold4 <- out2 %>% 
  filter(Compounds %in% threshold4)

# Convert to matrix (have to remove Biomarkers names then add them back in)
data_matrix_threshold4 <- selected_data_threshold4 %>%
  select(-Compounds) %>%
  as.matrix() %>%
  t()  # Transpose so biomarkers are on x-axis, biosets on y-axis

colnames(data_matrix_threshold4) <- selected_data_threshold4$Compounds
...
# Create the heatmap using the image function
par(mar = c(2, 10, 10, 0), cex.main = 0.8)  # Margins: bottom, left, top, right
image(1:ncol(data_matrix_threshold4), 1:nrow(data_matrix_threshold4), t(data_matrix_threshold4),
      col = custom_colors, breaks = c(-100, -4, 4, 100), axes = FALSE, xlab = "", ylab = "",
      main = "Heatmap for Biomarkers with Threshold -4 to 4")

# Add row and column labels
axis(3, at = 1:ncol(data_matrix_threshold4), labels = colnames(data_matrix_threshold4), las = 2)
axis(2, at = 1:nrow(data_matrix_threshold4), labels = rownames(data_matrix_threshold4), las = 2)

# Add borders around the cells
for (i in 1:ncol(data_matrix_threshold4)) {
  for (j in 1:nrow(data_matrix_threshold4)) {
    rect(i - 0.5, j - 0.5, i + 0.5, j + 0.5, border = "grey")
  }
}
...
par(mar = c(5, 0, 5, 0))  # Adjust margins for the legend
plot.new()
legend("center", legend = c("< -4", "-4 to 4", "> 4"), 
       fill = c("blue", "white", "red"), 
       title = "-log(p-value)")
```

```{r heatmap_thresholdunkown2, echo=FALSE, fig.height=heatmap_height}
selected_data_thresholdunknown <- out2 %>% 
  filter(Compounds %in% thresholdunknown)

# Exclude the 'Compounds' column and convert the remaining data frame to a matrix
data_matrix_thresholdunknown <- selected_data_thresholdunknown %>%
  select(-Compounds) %>%
  as.matrix()

# Set the row names of the matrix to the values in the 'Compounds' column
rownames(data_matrix_thresholdunknown) <- selected_data_thresholdunknown$Compounds

# Create the heatmap
pheatmap(data_matrix_thresholdunknown,
         cluster_rows = FALSE,
         cluster_cols = FALSE,
         show_rownames = TRUE,
         show_colnames = TRUE,
         ...
         cellwidth = 10,
         cellheight = 10,
         main = "Heatmap for Biomarkers with Unknown Threshold")
```

```{r heatmap_thresholdunknown, echo=FALSE, fig.height=heatmap_height}
# Assuming data_matrix_thresholdunknown is already defined and transposed if needed
selected_data_thresholdunknown <- out2 %>% 
  filter(Compounds %in% thresholdunknown)
...
colnames(data_matrix_thresholdunknown) <- selected_data_thresholdunknown$Compounds

# Set up the layout: 1 row, 2 columns, so legend is beside the heatmap
layout(matrix(c(1, 2), nrow = 1), widths = c(4, 1))

# Define a continuous color palette
continuous_colors <- colorRampPalette(c("blue", "white", "red"))(100)

# Create the heatmap using the image function
par(mar = c(2, 10, 14, 0), cex.main = 0.8)  # Margins: bottom, left, top, right
image(1:ncol(data_matrix_thresholdunknown), 1:nrow(data_matrix_thresholdunknown), t(data_matrix_thresholdunknown),
      col = continuous_colors, axes = FALSE, xlab = "", ylab = "",
      main = "Heatmap for Biomarkers with no published thresholds")

# Add row and column labels
axis(3, at = 1:ncol(data_matrix_thresholdunknown), labels = colnames(data_matrix_thresholdunknown), las = 2)
axis(2, at = 1:nrow(data_matrix_thresholdunknown), labels = rownames(data_matrix_thresholdunknown), las = 2)

# Add borders around the cells
for (i in 1:ncol(data_matrix_thresholdunknown)) {
  for (j in 1:nrow(data_matrix_thresholdunknown)) {
    rect(i - 0.5, j - 0.5, i + 0.5, j + 0.5, border = "grey")
  }
}
...
par(mar = c(5, 0, 5, 0))  # Adjust margins for the legend
plot.new()
legend("center", legend = c("Low", "Medium", "High"), 
       fill = c("blue", "white", "red"), # colorRampPalette(c("blue", "white", "red"))(100)??
       title = "Value")
```
